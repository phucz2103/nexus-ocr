...
def _install_einops_paddle_backend_shim() -> None:
    try:
        import einops._backends as einops_backends
        import paddle
    except Exception as exc:
        LOGGER.warning("Could not install einops Paddle backend shim: %s", exc)
        return

    if "paddle" in getattr(einops_backends, "_backends", {}):
        return

    backend_subclasses = []
    backends = einops_backends.AbstractBackend.__subclasses__()
    while backends:
        backend = backends.pop()
        backends += backend.__subclasses__()
        backend_subclasses.append(backend)

    for subclass in backend_subclasses:
        if getattr(subclass, "framework_name", None) == "paddle":
            einops_backends._backends["paddle"] = subclass()
            return

    class PaddleBackend(einops_backends.AbstractBackend):
        ...
        def from_numpy(self, x):
            return self.paddle.to_tensor(x)
        ...
        def reduce(self, x, operation, reduced_axes):
            for axis in sorted(reduced_axes, reverse=True):
                if operation == "min":
                    ...
                    raise NotImplementedError(f"Unknown reduction {operation}")
            return x
    ...
    einops_backends._backends["paddle"] = PaddleBackend()
    LOGGER.info("Installed einops Paddle backend compatibility shim")
    ...
    def _ensure_pipeline(self):
        ...
        with self._lock:
            ...
            try:
                ...
                _install_einops_paddle_backend_shim()
